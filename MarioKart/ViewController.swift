...
class ViewController: UIViewController,
    ...
    @IBAction func didDoubleTapKart(_ sender: UITapGestureRecognizer) {
        ...
        } // move the kart forward by the containing view width
    ...
    // location of the gesture in the view
    private func moveKart(using gestureRecognizer: UIPanGestureRecognizer) {
        ...
        kartView?.center = location //move kart to that location
    }
    ...
    private func resetKarts() {
        UIView.animate(withDuration: 0.4) {
            ...
            // reset kart positions to the start
    ...
    override func viewDidAppear(_ animated: Bool) {
        super.viewDidAppear(animated) // since overriding from super class
    ...
    private func runStartingAnimationsOneByOne(completion: (() -> Void)? = nil) {
        getKartReadyToRace(kart: kartView0) { // animate kartView0 first
            self.getKartReadyToRace(kart: self.kartView1) { // ...then kartView1}
                self.getKartReadyToRace(kart: self.kartView2) { // ..thenkartView2}
                    completion?()
                }
            }
        }
    ...
    private func raceKartsWithRandomizedSpeed() {
        let kartView0Speed = Double.random(in: 0.5...5) //get a random value between 0.5 and 5
        ...
        // Wait 2 seconds to race again!
        DispatchQueue.main.asyncAfter(deadline: .now() + max(kartView0Speed, kartView1Speed, kartView2Speed) + 1.0) {
            self.resetKarts()
